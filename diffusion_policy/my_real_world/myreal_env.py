...
from diffusion_policy.my_real_world.myinterpolation_controller import myInterpolationController
...
class RealEnv:
    def __init__(self, 
        ...
        robot = myInterpolationController(
            shm_manager=shm_manager,
            robot_ip=robot_ip,
            frequency=100, # UR5 CB3 RTDE
            lookahead_time=0.1,
            gain=300,
            max_pos_speed=10000,
            max_rot_speed=2,
    ...
    def exec_actions(self, 
        ...
        new_actions_absolute = []
        current_pose = self.robot.read_space_pose()

        for action in actions:
            new_pose = [current_pose[i] + action[i] for i in range(len(current_pose))]
            new_actions_absolute.append(new_pose)
            current_pose = new_pose  # Update current pose for the next action

        new_actions = np.array(new_actions_absolute)
        new_actions = new_actions[is_new]
